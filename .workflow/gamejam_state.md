...
| 六·B | 美术资产提示词 + 自动出图 + 审核 | 🟡 进行中（B.1 资产清单 v1.0 已定稿，22 条；B.2 批量 prompt 待启动） | — |
| 七 | 测试用例验收 | ⏳ 待执行 | — |
...
- **当前阶段**：阶段六·B.1 ✅ 关闭（v1.0 定稿，22 条）；阶段六·B.2（批量 prompt）待启动
- **阶段状态**：art_asset_list.md v1.0（22 条；🔴 20 / 🟡 1 / 🟢 0）；atoms/ 工程骨架已建立（落盘约定：`atoms/assets/art/<category>/<asset_id>.png`）
- **下一步**：
  - **第一动作**：启动 06b【交付 2/3：批量 prompt】，按场景 → 立绘 → 敌人 → UI → 结局 → 装饰 分批生成 `design/art_prompts/<asset_id>.md` 文件，每批输出后等用户抽查
  - **三人并行分工**（用户已确认，2026-05-19 决议）：
    - **1 人开发（Atoms）**：依据 balance v0.1.2 + levels v1.3 实现：战斗状态机 / 配置资源 / EventRollService 3 槽抽签 / B07 OnFirstHeroCrit hook / L03 必死阈值 + 兜底 / L06 行动点参数化；用纯色 placeholder 占位（按 §2.2 主色规则），按 asset_id 命名引用，等美术覆盖
...
- **今日进度（2026-05-20）**：
  ...
  - 阶段六·B.1：资产清单 v0.1（38 条草案）→ 用户拍板 6 项裁剪全接受 → v1.0（22 条；20 🔴 + 1 🟡）✅ 关闭
  - **去 Unity 化**：用户决定不用 Unity 改用 Atoms；创建 atoms/ 工程根目录骨架；落盘约定确定为 `atoms/assets/art/<category>/<asset_id>.png`；art_asset_list.md / gamejam_state.md / 06b skill / art_asset_list_template / art_prompt_template 路径替换完成（其余 Unity 残留按需清理策略 A 处理）
- **昨日进度（2026-05-19）**：
