...
以下是 Gemini 图像模型固有限制，**不视为 prompt 失败，不扣分；06d 不应因此判 🔴 触发 Pro 升级**：

| 固有限制 | Flash 表现 | Pro 表现 | 正确处理 |
|---------|-----------|---------|---------|
| 尺寸无法控制 | 恒 1024×1024 PNG | 不可预测（实测 1408×768 JPEG） | 只看构图对不对，绝对尺寸交 06c post-process |
| Alpha 不支持 | RGB PNG，无 alpha | JPEG，格式级无 alpha | 可用性维度看背景是否"便于 post-process 抠图"（纯色 / 高对比 / 主体边缘清晰） |
| Pro 返 JPEG | N/A | mimeType=image/jpeg | 06c post-process 会转 PNG，06d 不关心这个 |
| 轻微 C2PA 水印 metadata | 有 | 有 | 无视 |
| Pro 画"棋盘格"模拟透明 | 不会 | 偶尔会（当 prompt 含 "transparent"） | 标 🟡 "背景不洁净"，06b 的 prompt 规则已禁止写 transparent，偶发时走修正 |

### 重要：升级到 Pro 只能解决"主体细节缺失"

06d 给修正建议时，**明确区分以下两类问题**：

**可触发 Pro 升级（🔴 + confidence 高/中）**
...
若既有"主体缺失"又有"尺寸瑕疵"，可以判 🔴 升级（Pro 有希望解决主体，尺寸走 post-process）。
