# A-EMOTE-RANK-B — 绩效评级章 B

**类别**：装饰贴 / 表情贴（绩效评级章 系列）
**来源**：`design/art_asset_list.md` v1.1 §5.8 装饰贴（新增）+ `design/narrative.md` v1.3 T05 绩效备忘录红章
**引用场景**：A-UI-RESULT 波次结算面板绩效评级槽位的中档评级章；与 CEO-STAMP / RANK-S/A/C/D 同属红章家族
...
| 切图方式 | 单张装饰贴，直接用 |
| atoms 落位路径 | `atoms/assets/art/emotes/A-EMOTE-RANK-B.png` |
| pivot 位置 | Center |

## 风格锚点（继承自 art_style_guide v1.0）

- **系列共同点**：圆形磨损边橡皮章；中央抽象档位字符占位；与 CEO-STAMP 同一橡皮章设计语言但语义为"档位评级"
- **本档（B）独有点**：中档，**主色为主 3 灰薄荷绿 `#8FA89B`**（中性平稳，象征"达标"），章内字符为「B」形几何双圆瓣剪影（不可读拉丁字母）
- 情绪词：deadpan 公文式合格员工通过、平静温吞
- 色盘偏重：**主 3 灰薄荷绿 `#8FA89B`（B 档主色）** + 主 1 莫兰迪雾灰米 `#B8B5A8`（抠图底）+ 辅 1 描边深炭灰 `#3D3A36`
- 特殊注意：
  - 中央"B"字符以**抽象双圆瓣（叠加两个半圆）silhouette**呈现，绝非可读拉丁 B
  - 章外圈可有"★★★ 三颗小星几何剪影"环绕作为 B 档识别锚点
...
a single decorative office performance grading stamp seal mark for the mid-tier rank B, round to slightly irregular circular shape with authentic uneven worn rubber-stamp pressed-edge imperfections, solid dusty mint green #8FA89B as the dominant fill (mid-tier passing-grade color), centered composition with a single large abstract double-bowl silhouette made of two stacked semicircle shapes in the middle as the rank glyph placeholder, the double-bowl shape is drawn as a stylized geometric symbol never readable Latin letter B, surrounded by three tiny abstract star-like geometric silhouettes evenly arranged around the inner perimeter of the stamp ring as a visual cue of mid-tier rank, text shapes are abstract glyph placeholders, no readable letters, no readable numbers, faint subtle paper-fiber texture visible underneath but kept minimal so it never overpowers the mint imprint, ink imprint texture, slightly uneven edges, flat 2D vector style, centered composition, clean isolated stamp on a flat neutral Morandi foggy-beige #B8B5A8 background, ample empty space around subject for easy cutout, deadpan corporate mid-tier passing-grade mood, clean vector-flat finish
```
...
photorealistic stamp, gold foil, ornate borders, complex calligraphy, traditional Chinese seal, Japanese hanko intricate kanji, characters, portraits, blood spatter, readable letters, real text, crisp readable letter B, clearly legible Latin alphabet, sharp typeset "B" wordmark, "RANK B" text, embossed metallic finish, decorative filigree, multiple stamps overlapping, gradient ink, glossy reflection, 3D bevel, drop shadow under stamp, framed border decoration, brass gold color, terracotta orange color, brick red color
```

## 参考艺术家 / 作品（可选）

- 《Two Point Hospital》"合格"公文体扁平印章贴
- 现代企业季度绩效"B"等级章排版
...
- [ ] Gemini 出图符合正向 prompt：圆形磨损边灰薄荷绿印章 + 中央抽象 B 双瓣剪影 + 三颗小星点缀
- [ ] 未出现反向 prompt 禁忌：无可读拉丁 B、无其他档位色
- [ ] 主色为灰薄荷绿 `#8FA89B`
- [ ] 尺寸落盘 256×256
- [ ] 已按 atoms 落位路径 `atoms/assets/art/emotes/A-EMOTE-RANK-B.png` 保存
- [ ] 与 RANK-S/A/C/D 同一橡皮章设计语言，仅色 + 字符差异化

## 审核结论（06d 逐轮追加，不覆盖历史）

### Flash#1 @ <时间戳>
- 模型：`gemini-2.5-flash-image`
- 文件：`atoms/assets/art/emotes/A-EMOTE-RANK-B__v1__flash.png`
- 结论：🟢 / 🟡 / 🔴
