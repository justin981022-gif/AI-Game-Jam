...
| 切图方式 | 单张图标，不切片 |
| atoms 落位路径 | `atoms/assets/art/ui/A-UI-ICO-VOLUME.png` |
| pivot 位置 | Center |

## 风格锚点（继承自 art_style_guide v1.0）

- 情绪词：deadpan 公文式克制系统按钮、与 PAUSE / RESTART 同一套设计语言
- 色盘偏重：辅 1 描边深炭灰 `#3D3A36`（喇叭主体）+ 辅 2 米白 `#E8E2D5`（圆形底）+ 辅 4 黄铜金 `#D4A574`（外圈细装饰）+ 主 3 灰薄荷绿 `#8FA89B`（音波弧线点缀，区别 PAUSE/RESTART） + 主 1 莫兰迪雾灰米 `#B8B5A8`（抠图底）
- 特殊注意：
  - **圆角方形 / 正圆底**：与 PAUSE / RESTART 同款 96px 直径圆形底，米白填充 + 黄铜金 1px 细外圈
  - **音量符号本体**：左侧梯形喇叭（深炭灰填充）+ 右侧 2 道弧形音波（灰薄荷绿描边，约 2~3px 粗）
  - 不画 "ON / OFF" 文字或斜杠静音标记（静音态由代码 Tint 灰度切换）
  - 周围至少 16px 留白安全区
...
a single flat 2D system UI button icon for VOLUME function, centered composition, a clean round circular base disc filled with warm cream #E8E2D5 outlined by a thin uniform 5px charcoal #3D3A36 stroke and an extra 1px brass-gold #D4A574 hairline ring just outside the charcoal stroke, inside the disc centered are two elements: on the left a small simplified geometric trapezoidal speaker shape filled with solid charcoal #3D3A36 with no outline (a rectangle on the left and a wider trapezoid attached to its right forming the speaker silhouette), on the right two simple arc-shaped sound-wave curves radiating outward drawn as 2 to 3 pixel wide dusty mint green #8FA89B strokes, no slash mark across the icon, both elements perfectly centered together within the disc, pure flat fills with no gradients, NO chibi monsters, NO characters, NO mascots, NO text, NO letters, NO numbers, NO additional decoration beyond the speaker and two wave arcs, clean isolated icon on a flat neutral solid grey #B8B5A8 background, ample empty space around subject for easy cutout, vector-flat finish, deadpan corporate system-icon mood
```
...
photograph, 3D bevel, glossy reflection, glowing button, gradient ink, drop shadow, neon glow, gemstones, decorative filigree, additional icons, mute slash mark across the speaker, "VOLUME" wordmark, "MUTE" text, readable letters, chibi monsters, characters, mascots, photographic speaker, skeuomorphic UI, embossed metallic finish, music notes, headphone illustration
```

## 参考艺术家 / 作品（可选）

- macOS / iOS 系统音量图标（扁平克制）
- Material Design 音量图标
- 《Two Point Hospital》系统音量按钮（极简公文体）
...
- [ ] Gemini 出图符合正向 prompt：圆形底 + 喇叭剪影 + 2 道弧形音波
- [ ] 未出现反向 prompt 禁忌：无文字、无静音斜杠、无角色
- [ ] 尺寸落盘 128×128（post-process 后）
- [ ] 已按 atoms 落位路径 `atoms/assets/art/ui/A-UI-ICO-VOLUME.png` 保存
- [ ] 与 A-UI-ICO-PAUSE / A-UI-ICO-RESTART 视觉一致（同一圆形底设计语言）

## 审核结论（06d 逐轮追加，不覆盖历史）

### Flash#1 @ <时间戳>
- 模型：`gemini-2.5-flash-image`
- 文件：`atoms/assets/art/ui/A-UI-ICO-VOLUME__v1__flash.png`
- 结论：🟢 / 🟡 / 🔴
