...
- 模型：aiart 默认 artSpec
- 文件：`atoms/assets/art/ui/A-UI-RESULT.png`（覆盖 v1；__v2__aiart.png 保留历史）
- 触发：与 A-UI-CARD-EVENT 同次驳回（atoms 主界面截图 LOREM IPSUM 泄漏）；v1 正向 prompt 自身要求"only LOREM IPSUM placeholder text shown as light grey horizontal bars to imply data fields"
- 修订点：
  - 正向 prompt：删除"only LOREM IPSUM placeholder text"句，改为"ABSOLUTELY NO TEXT of any kind anywhere on the panel... pure flat cream surface ready for code-side text overlay"
  - 反向 prompt：增加"ANY TEXT, ANY LETTERS, lorem ipsum, LOREM IPSUM, dummy text, placeholder text, latin words, chinese characters, faux text, grey horizontal text bars, tabular text rows..."等强禁忌
