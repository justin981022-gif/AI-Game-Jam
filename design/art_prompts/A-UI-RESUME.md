...
flat 2D UI element, corporate paperwork aesthetic, centered composition, 9-slice border layout, an empty single vertical job-application resume card frame in 3:4 portrait ratio, warm cream #E8E2D5 paper fill across the entire body, uniform 5px charcoal #3D3A36 outline with slightly rounded 16px corners, the top section approximately 100px tall hosts a title-bar zone rendered as TWO solid foggy-beige #B8B5A8 horizontal placeholder bars stacked vertically with a single thick brass-gold #D4A574 accent stripe between them, ABSOLUTELY no Latin letters, no English words, no R-E-S-U-M-E shapes, no glyphs of any kind in the title area, immediately below the header a centered 200x200 px rounded-square portrait reservation slot filled with a flat foggy-beige #B8B5A8 tone and outlined in charcoal at 5px, leave this slot empty so a character portrait can be composited later, below the portrait slot draw five horizontally stacked light grey LOREM IPSUM placeholder bars of varying widths to imply name / role / experience / skills fields rendered ONLY as plain grey rectangles never as letters, leave generous vertical spacing between bars, the bottom 80px area shows a single thin charcoal hairline underline ONLY representing where a signature would go, NO cursive word, NO "Signature" lettering, NO calligraphic handwriting of any kind, plus a small empty circular seal reservation outline (do not fill the seal), brass-gold tiny corner ornaments at lower-left and upper-right within a 32px corner safe zone, no real readable text anywhere on the entire canvas, only placeholder grey bars and hairlines, clean isolated UI on a flat neutral solid grey #B8B5A8 background, ample empty space around subject for easy cutout, vector-flat finish, no perspective, deadpan office-document mood
```
