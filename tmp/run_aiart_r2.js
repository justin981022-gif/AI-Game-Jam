// R2 重出: A-CHR-HR (女性版) + A-UI-CARD-EVENT (空框) + A-UI-RESUME (无文字)
...
const TARGETS = [
  { id: 'A-CHR-HR',        cat: 'characters', tw: 768,  th: 1024, transparent: true,  v: 2 },
  { id: 'A-UI-CARD-EVENT', cat: 'ui',         tw: 900,  th: 600,  transparent: true,  v: 2 },
  { id: 'A-UI-RESUME',     cat: 'ui',         tw: 600,  th: 800,  transparent: true,  v: 2 },
];
...
async function submit(p, n) {
  ...
  const res = await req({ method: 'POST', hostname: HOST, path: BASE,
    headers: { 'Authorization': 'Bearer ' + KEY, 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } }, body);
  if (res.status !== 200) throw new Error('submit ' + res.status + ' ' + res.body.toString().slice(0,200));
...
async function postProcess(srcPath, asset) {
  ...
  const bw=Math.min(W-bx,(maxX-minX)+2*PAD+1), bh=Math.min(H-by,(maxY-minY)+2*PAD+1);
...
(async () => {
  ...
  for (const t of TARGETS) {
    ...
    tasks.push(submit(p.positive, p.negative).then(tid => {
      console.log(`[${t.id}] task=${tid}`);
      return { t, tid };
    }));
  }
  const submitted = await Promise.all(tasks);

  // poll + download in parallel
  await Promise.all(submitted.map(async ({ t, tid }) => {
    try {
      const url = await poll(tid);
      const raw = path.join(ART_DIR, t.cat, `${t.id}__v${t.v}__aiart.jpg`);
      await download(url, raw);
      const r = await postProcess(raw, t);
      console.log(`[${t.id}] R2 OUT ${path.basename(r.dst)} (${r.bytes}B alpha=${r.alpha})`);
    } catch (e) {
